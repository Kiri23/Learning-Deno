...
const timeData: TimeData[] = Array.from({ length: 50 }, (_, i) => ({
  time: i,
  value: Math.sin(i * 0.1) * 10 + Math.random() * 5,
}));

const scatterData: ScatterData[] = Array.from({ length: 100 }, () => ({
  x: Math.random() * 100,
  y: Math.random() * 100,
  size: Math.random() * 20 + 5,
}));

// Create plots with full TypeScript support
const barChart = Plot.plot({
  marks: [Plot.barY(data, { x: "category", y: "value", fill: "steelblue" })],
  width: 600,
  height: 300,
  margin: 40,
});
...
export function renderPlots(): void {
...
// Export individual plots for flexibility
export { barChart, lineChart, scatterPlot, data, timeData, scatterData };
export type { BarData, TimeData, ScatterData };
